...
El panel LED seleccionado (Sección \ref{subsec:luces_cultivo}) es una fuente de emisión rectangular, no puntual, con una placa estructural de $17\text{ cm} \times 11{,}3\text{ cm}$. Bajo el criterio de diseño de mantener el dosel vegetal a una distancia de la luminaria de entre 20 y 30 cm, y considerando el ángulo de incidencia de $120°$ especificado por el fabricante, la huella iluminada en el plano de la maceta --para una fuente rectangular-- se obtiene sumando a cada lado del panel la expansión lateral del haz:
...
La calibración del factor de luminaria del panel LED se realizó mediante una medición directa con el sensor BH1750, colocando la luminaria a una altura de 27 cm respecto del dosel vegetal --dentro del rango de instalación de 20 a 30 cm establecido en la Sección \ref{subsec:diseno_geometrico_luz}--. La lectura obtenida en esa configuración, de $6500\ \si{lux}$, se adoptó como el valor de referencia de iluminancia del foco ($lux_{foco}$) empleado en el algoritmo de control (Sección \ref{subsec:algoritmo_control_dli}). A partir de esta calibración, y dado que la luz solar posee una composición espectral distinta a la del panel LED, se determinaron dos factores de luminaria independientes: $F_L = 0{,}0887$ para el foco artificial, y $F_{L,\text{ambiente}} = 0{,}0185$ para la luz natural.

Antes de su implementación en el software de control, la conversión de lux a PPFD y su posterior integración a DLI (Ecuaciones \ref{eq:ppfd_lux} y \ref{eq:dli_ppfd} del Marco Teórico) se validó numéricamente mediante un script de cálculo, tanto en su forma directa (a partir de una lectura de iluminancia, obtener el DLI resultante) como en su forma inversa (a partir de un DLI objetivo, obtener la iluminancia de luxómetro necesaria):

\begin{verbatim}
Ti = 12;            % Fotoperiodo en horas
Lux = 6500;         % Lectura de referencia del foco (BH1750, h = 27 cm)
F_L = 8.87/100;     % Factor de luminaria del foco
...
Para el valor de referencia obtenido en la calibración ($Lux = 6500$, $F_L = 0{,}0887$) y un fotoperíodo hipotético de $T_i = 12\text{ h}$, la conversión directa arroja $PPFD = 576{,}55\ \si{\micro\mole\per\square\meter\per\second}$ y $DLI \approx 24{,}91\ \si{\mole\per\square\meter\per\day}$ si el foco permaneciera encendido durante todo ese lapso --valor que ilustra la capacidad máxima de aporte del foco y no un tiempo de encendido real, dado que este se determina dinámicamente mediante el algoritmo de la Sección \ref{subsec:algoritmo_control_dli}--. La conversión inversa, para un objetivo de $DLI_D = 17{,}3\ \si{\mole\per\square\meter\per\day}$, indica que se requeriría una lectura sostenida de $Luxs \approx 4515\ \si{lux}$ durante todo el fotoperíodo para alcanzar exactamente ese objetivo. Esta verificación permitió confirmar la consistencia de ambos factores de luminaria antes de trasladar la lógica al lazo de control principal.
...
El algoritmo distingue dos factores de luminaria calibrados de forma independiente --uno para el panel LED ($F_L$) y otro para la luz solar ($F_{L,\text{ambiente}}$)--, dado que ambas fuentes tienen composiciones espectrales distintas y, por lo tanto, requieren constantes de conversión propias. En cada ciclo de control, de duración $\Delta t$:
...
\begin{enumerate}
    ...
    \item Se calcula el PPFD aportado por el ambiente, a partir de la lectura en vivo del sensor BH1750: $PPFD_{ambiente} = lux_{ambiente} \cdot F_{L,\text{ambiente}}$.
    \item Se obtiene el PPFD total del ciclo, sumando el aporte ambiente con el del foco únicamente si este se encuentra encendido: $PPFD_{total} = PPFD_{ambiente} + PPFD_{foco} \cdot \mathbb{1}_{\text{foco encendido}}$.
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{Conexionado de cables por color}
    \Fuente{propia.}
\end{figure}
