...
def _parsear_configuracion(data: dict) -> SystemConfig:
    ...
    for nombre_maceta, maceta_data in macetas_data.items():
        ...
        macetas[nombre_maceta] = MacetaConfig(
            ...
            umbral_humedad_suelo_raw=maceta_data["umbral_humedad_suelo_raw"],
            ...
            # Nuevas variables DLI inyectadas al modelo
            hora_inicio_dia=maceta_data.get("hora_inicio_dia", 10),
            hora_fin_dia=maceta_data.get("hora_fin_dia", 24),
            dli_objetivo=maceta_data.get("dli_objetivo", 14.0),
            lux_foco=maceta_data.get("lux_foco", 6500.0),
            factor_luminaria=maceta_data.get("factor_luminaria", 0.0887),
            factor_luminaria_ambiente=maceta_data.get("factor_luminaria_ambiente", 0.0185),
...
def _validar_macetas(config: SystemConfig) -> None:
    for nombre_maceta, maceta in config.macetas.items():
        if not (0 <= maceta.umbral_humedad_suelo_raw <= 255):
            raise ValueError(f"{nombre_maceta}: umbral_humedad_suelo_raw fuera de rango")

        if not (0 <= maceta.umbral_humedad_ambiente_raw <= 255):
            raise ValueError(f"{nombre_maceta}: umbral_humedad_ambiente_raw fuera de rango")

        if maceta.tiempo_riego_seg < 0:
            raise ValueError(f"{nombre_maceta}: tiempo_riego_seg no puede ser negativo")

        if not (0 <= maceta.hora_inicio_dia <= 24):
            raise ValueError(f"{nombre_maceta}: hora_inicio_dia fuera de rango (0-24)")

        if not (0 <= maceta.hora_fin_dia <= 24):
            raise ValueError(f"{nombre_maceta}: hora_fin_dia fuera de rango (0-24)")

        if maceta.dli_objetivo <= 0:
            raise ValueError(f"{nombre_maceta}: dli_objetivo debe ser mayor a 0")

        if maceta.lux_foco < 0:
            raise ValueError(f"{nombre_maceta}: lux_foco no puede ser negativo")

        if maceta.factor_luminaria <= 0:
            raise ValueError(f"{nombre_maceta}: factor_luminaria debe ser mayor a 0")
        if maceta.factor_luminaria_ambiente <= 0:
            raise ValueError(f"{nombre_maceta}: factor_luminaria_ambiente debe ser mayor a 0")  
