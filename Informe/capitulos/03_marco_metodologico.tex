% ============================================================
% PARTE II - MARCO METODOLÓGICO (3 capítulos)
% ============================================================
...
% ================================================================
% CAPÍTULO 1 — DISEÑO ESTRUCTURAL Y ANÁLISIS DE SISTEMAS INTEGRADOS
% ================================================================
\chapter{Concepción y diseño del prototipo}
\label{cha:diseno_estructural}

\textit{Este capítulo presenta el plan de trabajo y los instrumentos empleados durante el desarrollo del proyecto, la selección de los cultivos, la unidad de procesamiento central y los componentes del sistema, y la arquitectura mecánica del prototipo (prototipos inicial y final, impermeabilización y drenaje, modularidad, y piezas diseñadas mediante manufactura aditiva).}

\section{Plan de trabajo e instrumentación}
\label{sec:plan_trabajo_instrumentacion}

Para el diseño, construcción y validación del prototipo de cultivo automatizado, se siguió un plan de ejecución estructurado, junto con el cronograma de actividades y los instrumentos y herramientas empleados durante su desarrollo.
...
La selección de la unidad de procesamiento central constituyó una decisión estratégica orientada a garantizar la modularidad y la proyección a largo plazo del sistema. En lugar de optar por microcontroladores tradicionales de arquitectura básica (como las familias ATmega o los módulos ESP), se seleccionó la computadora de placa única (\textit{Single Board Computer} o SBC) Raspberry Pi 4 Model B. Esta determinación se fundamentó en la previsión de un crecimiento modular del sistema, el cual contempla la adición de múltiples módulos de cultivo y macetas automatizadas sin un límite predefinido, asegurando que la capacidad de cómputo y el procesamiento de datos concurrentes no representen un cuello de botella tecnológico.
...
Durante la fase experimental, se realizaron pruebas preliminares y validaciones técnicas que permitieron identificar oportunidades de optimización en el diseño original. A partir de los datos recolectados en estas iteraciones, se aplicaron ajustes correctivos para maximizar la eficiencia operativa del sistema. Una vez validados los criterios de funcionamiento bajo los estándares previstos, se procedió al diseño e implementación del prototipo final, integrando arquitecturas de hardware y software.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Imagenes/prot12.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Imagenes/prot25.png}
    \end{minipage}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Imagenes/prot4.png}
...
% ================================================================
% CAPÍTULO 3 — SOFTWARE Y SISTEMAS DE CONTROL
% ================================================================
\chapter{Software de control}
\label{cha:software_control}

\textit{Este capítulo describe la arquitectura del software de control local, la lógica de las rutinas de control implementadas, su integración con la plataforma de telemetría en la nube ThingSpeak, y la interfaz de usuario del sistema.}
...
\begin{itemize}
    ...
    \item \texttt{hardware.py}: Establece la capa de abstracción de hardware (Sección \ref{subsec:capa_hardware_real}), traduciendo operaciones de alto nivel (``leer humedad de suelo'', ``encender luz'') a las operaciones concretas de bus I2C y de pines GPIO.
...
La totalidad del código fuente descripto en este capítulo, junto con los archivos de configuración y los scripts auxiliares de análisis numérico empleados durante el desarrollo del prototipo, se encuentra disponible públicamente en el repositorio del proyecto \cite{repo}.
% Recuerda agregar esta línea en tu preámbulo (antes de \begin{document}):
% \usepackage{float}
...
% Aumentamos el valor Y para separar verticalmente (de 7.2 a 11.5)
\node (models) [modulo, fill=blue!8] at (3.6,11.5) {\textbf{models.py}\\ \textit{Dataclasses:} SystemConfig, MacetaConfig, MacetaEstado, GlobalConfig};

% Aumentamos el valor Y (de 5.2 a 8.0)
\node (configloader) [modulo, fill=blue!8] at (3.6,8.0) {\textbf{config\_loader.py}\\ cargar\_configuracion()\\ \_validar\_configuracion()};

% Lo movemos hacia la izquierda (X de 0 a -2) y ajustamos Y
\node (hardware) [modulo, fill=orange!10, text width=3.9cm] at (-2,3.8) {\textbf{hardware.py}\\ \textit{HardwareManager:}\\ leer\_humedad\_raw()\\ leer\_lux(), leer\_dht()\\ set\_luz\_maceta()\\ set\_valvula\_maceta()\\ set\_ventilador\_maceta()};

% Lo movemos hacia la derecha (X de 7.2 a 9.2) y ajustamos Y
\node (control) [modulo, fill=orange!10, text width=3.9cm] at (9.2,3.8) {\textbf{control.py}\\ procesar\_maceta()\\ calcular\_y\_controlar\_dli()\\ decidir\_riego()\\ decidir\_ventilacion()};

% Este se mantiene como base en Y=0
\node (main) [modulo, fill=green!10] at (3.6,0) {\textbf{main.py}\\ main(): ciclo principal\\ ejecutar\_riego\_seguro()\\ guardar\_csv(), subir\_thingspeak()};

% Las conexiones se mantienen iguales, TikZ las redibujará con las nuevas distancias
...
\end{tikzpicture}
\caption[Dependencias de importación entre los módulos del proceso de control.]{Dependencias de importación entre los módulos del proceso de control. Todas las flechas representan relaciones de código (\textit{import}): \texttt{main.py} depende de los tres módulos restantes, que a su vez comparten las estructuras de datos definidas en \texttt{models.py}.}
...
\begin{lstlisting}[title={Ejemplo de configuración para una maceta (config.toml)}]
[macetas.maceta1]
...
\begin{enumerate}
    ...
    \item Transcurrido el tiempo de riego configurado, la bomba se apaga antes que la válvula, y se introduce una segunda espera (el tiempo de despresurización configurado) antes de cerrar la válvula, evitando que la manguera quede presurizada tras el cierre.
\end{enumerate}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Imagenes/Screenshot 2026-08-09 203535.png}
\caption[Vista de la interfaz de control desde el navegador web.]{Vista de la interfaz de control desde el navegador web en una computadora: Formulario de ajuste de parámetros (\texttt{config.toml}). Fuente: Elaboración propia.}
\label{fig:web_parametros}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Imagenes/Screenshot 2026-08-09 203555.png}
\caption[Vista de la interfaz de monitoreo desde el navegador web.]{Vista de la interfaz de monitoreo desde el navegador web en una computadora: Gráficos históricos de humedad de suelo e intensidad de luz. Fuente: Elaboración propia.}
\label{fig:web_historico_1}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{Imagenes/Screenshot 2026-08-09 203609.png}
\caption[Vista de la interfaz de monitoreo desde el navegador web.]{Vista de la interfaz de monitoreo desde el navegador web en una computadora: Gráficos históricos de temperatura y humedad ambiente, desplegada mediante el túnel inverso en \url{https://democrat-unrobed-excess.ngrok-free.dev/?success=1}. Fuente: Elaboración propia.}\label{fig:web_historico_2}
\end{figure}

% ================================================================
% SÍNTESIS DEL MARCO METODOLÓGICO
% ================================================================
\section*{Síntesis del marco metodológico}
\addcontentsline{toc}{section}{Síntesis del marco metodológico}

A lo largo de esta parte se reconstruyó el proceso completo de diseño, construcción y puesta en marcha del prototipo, siguiendo una progresión lógica desde la planificación hasta la operación autónoma del sistema. Se partió de un plan de trabajo y una selección de instrumentos que enmarcaron temporal y técnicamente el desarrollo; sobre esa base se definieron los criterios agronómicos, el hardware de procesamiento y la arquitectura mecánica del prototipo, resolviendo mediante iteraciones sucesivas los desafíos de impermeabilización, drenaje y modularidad. A partir de esa estructura física, se implementó el hardware de control --actuadores, etapas de potencia y esquemas de conexión-- dimensionado mediante cálculos específicos para los subsistemas de riego, iluminación y ventilación. Finalmente, el software desarrollado integró estos subsistemas bajo una arquitectura modular, dotando al prototipo de capacidad de control autónomo, registro histórico y monitoreo remoto.

En conjunto, este recorrido metodológico --de la planificación al diseño físico, de este al hardware, y del hardware al software-- constituye el sustento técnico sobre el cual se evaluarán, en la siguiente parte, los resultados obtenidos durante la operación del prototipo.